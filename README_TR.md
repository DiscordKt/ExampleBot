<p align="center">
   <a href="https://kotlinlang.org/">
   <img alt="Kotlin" src="https://img.shields.io/badge/Kotlin-1.7.10-%23A97BFF.svg?logo=Kotlin">
   </a>
   <a href="https://github.com/DiscordKt/DiscordKt">
   <img alt="DiscordKt" src="https://img.shields.io/badge/DiscordKt-0.23.4-%23E15ABA.svg?logo=data:image/png;base64,">
   </a>
   <br>
   <a href="https://discordapp.com/users/254786431656919051/">
   <img alt="Discord JakeyWakey#1569" src="https://img.shields.io/badge/Personal-JakeyWakey%231569-%2300BFFF.svg?logo=discord">
   </a>
</p>

Bu, DiscordKT kütüphanesi için tüm temel özelliklerini ve sözdizimini gösteren bir örnek bot projesi.

## Discord Kurulum

Örnek kodu çalıştırmadan önce Discord botu oluşturmalısınız.

### Bot Hesabı

CDiscord web sitesinin [geliştirici](https://discordapp.com/developers/applications/me) kısmından bir bot oluşturun

- Bir application oluşturun
- "General Information" sekmesinin altında
    - Bir ikon ve isim girin.
    - Botu Discord sunucunuza eklerken "client ID"ye ihtiyaç duyacaksınız.
- "Bot" sekmesinin altında
    - Botu oluşturun.
    - Bota bir kullanıcı adı ve profil fotoğrafı ekleyin.
    - Botu kullanırken botun "token"ine ihtiyacınız var.
        - "Token", gizli bir bilgidir, ifşa etmeyin!
- Değişiklikleri kaydedin

### Botu Ekleme

- Discord'un [yetkiler](https://discordapi.com/permissions.html) sayfasını ziyaret edin.
- "OAth URL Generator" sekmesinin altında botunuzun "client ID"sini girin.
- Botu sunucuya eklemek için verilen linke tıklayın.
